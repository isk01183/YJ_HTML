...
(async()=>{
 ...
 try{
  ...
  for(const lang of ['ko','ja','en']){
   await page.goto(url('collection_circle.html')+'?theme=ref-A01&lang='+lang+'&battery=78&run=123');
   assert.equal(await page.locator('[data-direct-circle="A01"]').count(),1,'Inline paths must be ready before native playback');
   ...
   assert.equal(await page.locator('#direct-art').evaluate(n=>getComputedStyle(n).transform),'none','No whole-image spin/zoom');
