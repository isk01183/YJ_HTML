...
(async () => {
    ...
    try {
        ...
        assert.equal(await page.locator('.card').count(),designs.list.length+119);
        ...
        assert.equal(await page.locator('[data-theme="native-N01"] .applied-badge').isVisible(),true);
        ...
        assert.equal(await page.locator('[data-group]').count(),1,'The unified gallery keeps only the All tab by default');
        assert.equal(await page.locator('.card:visible').count(),designs.list.length+119);
        await page.screenshot({path:path.join(__dirname,'../gallery-mobile.png')});
        await page.locator('[data-language="ko"]').click();
        await page.locator('#search').fill('벚꽃의 결인');
