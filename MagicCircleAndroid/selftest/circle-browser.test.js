...
(async () => {
    ...
    try {
        ...
        await page.goto(pathToFileURL(path.join(assets,'gallery.html')).href);
        assert.equal(await page.locator('.card').count(),13);
        ...
        for (const theme of designs.list.filter(theme=>theme.id!=='classic')) {
            await page.goto(pathToFileURL(path.join(assets,'theme_circle.html')).href+'?theme='+theme.id+'&battery=78&temperature=280&health=2&status=2&plugged=2');
            await page.evaluate(()=>{
        ...
        console.log('BROWSER_DESIGNS_OK: selection, 12 themes, real/unknown battery, fixed stars, 7s, tablet');
