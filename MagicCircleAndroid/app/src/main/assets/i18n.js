(function (root) {
    ...
    add('disableDesign', 'このデザインを無効化', '이 도안 비활성화', 'Disable this design');
    add('enableDesign', '有効にする', '활성화', 'Enable');
    add('manageInactive', '非表示デザインを管理', '비활성 도안 관리', 'Manage inactive designs');
    add('inactiveHint', 'コードまたは名前で検索し、デザインを個別に有効化できます。', '코드나 이름으로 찾아 도안을 하나씩 활성화할 수 있습니다.', 'Search by code or name and enable designs individually.');
    add('inactiveSearch', '無効なデザインを検索', '비활성 도안 검색', 'Search inactive designs');
    add('createTab', '＋ タブを作成', '＋ 탭 만들기', '＋ Create tab');
    add('renameTab', 'タブ名を変更', '탭 이름 변경', 'Rename tab');
    add('deleteTab', 'タブを削除', '탭 삭제', 'Delete tab');
    add('manageTabMembers', 'デザインを整理', '도안 넣기/빼기', 'Organize designs');
    add('tabEditor', 'ユーザータブ編集', '사용자 탭 편집', 'Edit custom tab');
    add('tabName', 'タブ名', '탭 이름', 'Tab name');
    add('tabNameInvalid', '1〜40文字で入力してください。', '1~40자로 입력해 주세요.', 'Enter 1–40 characters.');
    add('tabNameDuplicate', '同じ名前のタブがあります。', '같은 이름의 탭이 있습니다.', 'A tab with this name already exists.');
    add('save', '保存', '저장', 'Save');
    add('cancel', 'キャンセル', '취소', 'Cancel');
    add('tabMembers', 'タブのデザイン', '탭 도안 관리', 'Tab designs');
    add('tabMembersHint', '同じデザインを複数のタブに追加できます。', '하나의 도안을 여러 탭에 넣을 수 있습니다.', 'A design can belong to more than one tab.');
