...
(function(root){
  ...
  function sixfold(id){
    ...
    if(alt && k===1){effects=Array.from({length:14},(_,i)=>{const a=i*137.5,[x,y]=polar(88+i%4*3,a);return alt?at(snow(6),x-100,y-100):turn(p('M100 10L103 3L101 1L107-6','stroke-width=".7"'),a);}).join('');ornaments+=Array.from({length:8},(_,i)=>{const[x,y]=polar(89+i%2*6,i*45+15);return g(crystal(x,y,9),`stroke="#aeefff"`);}).join('');}
    if(alt && k===2){effects=orbit(98,28,20)+orbit(98,28,-20)+orbit(97,35,70);ornaments+=Array.from({length:12},(_,i)=>{const[x,y]=polar(92,i*30);return alt?spark(x,y,i%3?2:6,'fill="#fff2c5"'):leaf(x,y,5,i*30,'#fff2c5');}).join('');}
    if(alt && k===3){effects=spiral(7,96,.44,78);ornaments+=Array.from({length:12},(_,i)=>{const[x,y]=polar(90+i%3*3,i*30+10);return crystal(x,y,7+i%3);}).join('');}
    if(alt && k===4){effects=orbit(93,81,20)+orbit(97,87,-15);ornaments+=Array.from({length:25},(_,i)=>{const[x,y]=polar(90+i%3*3,i*137.5);return leaf(x,y,3.5,i*47,alt?'#ceefa0':'#9fffd3')+(i%3===0?flower(x+3,y,4,'#ffffd0'): '');}).join('');}
    if(alt && k===5){effects=orbit(100,43,32)+orbit(100,42,-28)+orbit(97,63,70);ornaments+=[[17,130,7],[175,45,6],[155,165,10]].map(([x,y,s])=>c(x,y,s,`fill="url(#${id}-planet)"`)+moons(x,y,s)).join('');ornaments+=p('M9 45L36 18L78 9L129 15L174 43L190 95M15 155L45 181L97 190L159 176','opacity=".6"');}
    return {color,outer,runes,geometry,core,ornaments,effects,paper:false};
  }
  ...
  function svg(id,options={}){
    ...
    if(id[0]==='W')return worldTree(id,options);
    const s=['A','B'].includes(id[0])?collection(id):['E','U'].includes(id[0])?early(id):sixfold(id);
    const dusty=!s.paper&&id!=='E08'&&id[0]!=='F',title=id+' · 직접 벡터 재구성';
    ...
    return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 200 200" preserveAspectRatio="xMidYMid meet" role="img" data-direct-circle="${id}" aria-label="${title}"><title>${title}</title>${defs(id,s.color)}${p('M0 0H200V200H0Z','fill="#02080f"')}${s.paper?r(94,'fill="#fafaf1"'):r(99,`fill="url(#${id}-wash)"`)}${drawLayer(id,'outer-ring',s.outer,s.color,s.paper?.65:.5)}${drawLayer(id,'rune-ring',s.runes,s.color,.35)}${drawLayer(id,'geometry',s.geometry,s.color,.5,true)}${drawLayer(id,'center-core',s.core,s.color,.8,true)}${drawLayer(id,'ornaments',s.ornaments,s.color,.7,true)}${drawLayer(id,'atmosphere',id[0]==='F'?'':s.effects,s.color,.5)}${drawLayer(id,'particles',dusty?particles(id,id[0]==='G'?170:70):'',s.color,.2)}</svg>`;
  }
